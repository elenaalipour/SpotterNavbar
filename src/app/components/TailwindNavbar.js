...
import { PiTruck, PiShield, PiHouse ,PiCaretDown , PiGoogleChromeLogo,PiCaretUp , PiCalculator,PiFileTextDuotone ,PiTrendUpFill , PiQrCode, PiPresentationChart     } from "react-icons/pi";

export default function TailwindNavbar() {
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
    const [openDropdown, setOpenDropdown] = useState(null);

    const toggleDropdown = (labelّ) => {
        setOpenDropdown((prev) => (prev === labelّ ? null : labelّ)); 
    };
    const menuItems = [
    ...
    return (
        <nav className="bg-white/5 backdrop-blur-lg px-8 py-4 flex items-center justify-between">
            {/* Logo */}
            ...
            <ul className="hidden lg:flex items-center gap-6">
                {menuItems.map((item) => (
                    <li key={item.label} className="relative group cursor-pointer py-2.5">
                        ...
                        {/* Dropdown */}
                        {item.children && (
                            <ul className="absolute left-1/2 -translate-x-1/2 top-full mt-2
transition-all duration-300 ease-out
opacity-0 group-hover:!opacity-100
invisible group-hover:!visible
translate-y-3 group-hover:translate-y-0
bg-black bg-opacity-30 dark:bg-background-6 rounded-[13px] px-[6px] py-[6px]
z-50 min-w-[280px] overflow-hidden
dark:bg-background-7 border border-stroke-1 dark:border-stroke-6
">
                           ...
                           hover:bg-[linear-gradient(135deg,_#40e0d0,_#008080)] hover:!text-white rounded-[13px] transition duration-100 group"
                                        >
                                            <span>{child.label}</span>
                                            <span className="opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-2 transition duration-200">
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    width="16"
                                                    height="16"
            ...
            <div className="hidden lg:block">
                <button className="px-4 py-2 rounded-lg font-semibold text-white signup-button">
                    Sign Up
                </button>
            </div>
            {/* Mobile Menu*/}
            <div
                className={`fixed py-[75px] top-0 h-screen left-0 w-full bg-[#043344] backdrop-blur-sm transform transition-transform duration-500 ease-in-out lg:hidden ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
                <div className="flex flex-col justify-center items-center h-full gap-3 text-white text-sm">
                    {menuItems.map((item) => (
                        <div key={item.label} className="w-full flex flex-col items-start">
                            <div
                                className="w-full flex justify-between items-center py-2 px-4 hover:bg-white/10 rounded cursor-pointer transition"
                            ...
                            >
                                ...
                                {item.children && (
                                    <span>{openDropdown === item.label ? <PiCaretUp  /> : <PiCaretDown  />}</span>
                                )}
                            </div>
                            {item.children && openDropdown === item.label && (
                                <div className="flex flex-col w-[98%] mx-2 mt-1 gap-2 rounded-[13px] border border-stroke-1 bg-black bg-opacity-50">
                                    {item.children.map((child) => (
                                        <a
                                            key={child.label}
                                            href={child.href}
                                            onClick={() => setIsMobileMenuOpen(false)}
                                            className="text-xs no-underline hover:bg-[linear-gradient(135deg,_#40e0d0,_#008080)] py-2 px-4 rounded-[13px] hover:bg-white/20 transition"
                                        >
                                            {child.label}
                                        </a>
                                    ))}
                    ...
                    <button
                        className="px-4 py-2 rounded-lg font-semibold text-white signup-button"
                        onClick={() => setIsMobileMenuOpen(false)}
                    >
                        Sign Up
                    </button>
